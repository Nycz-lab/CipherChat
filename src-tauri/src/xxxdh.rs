...
use cryptimitives::key::x25519_ristretto::{Signature, SecretKey, PublicKey};
use cryptraits::{
    aead::Aead,
...
impl Protocol
{
    ...
    pub fn prepare_init_msg(
        ...
        receiver_identity.verify(&receiver_prekey.to_vec(), &receiver_prekey_signature).unwrap();
        let ephemeral_key: cryptimitives::key::x25519_ristretto::KeyPair = cryptimitives::key::x25519_ristretto::KeyPair::generate_with(OsRng).into();
    ...
    fn _derive_sk(
        &self,
        source_data: [(&SecretKey, &PublicKey); 4],
    ) -> XxxDhResult<Vec<u8>> {
